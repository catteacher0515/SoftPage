...
test('renders fixed cover layout with hero image, title, author and divider', () => {
  ...
  expect(screen.getByTestId('cover-accent')).toBeInTheDocument()
  expect(screen.getByTestId('cover-accent')).toHaveStyle('width: 40px')
  expect(screen.getByTestId('cover-accent')).toHaveStyle('height: 10px')
  ...
  expect(screen.getByTestId('cover-hero')).toHaveStyle(
    'background-image: url(data:image/png;base64,abc)',
  )
  expect(screen.getByTestId('cover-hero')).toHaveStyle('background-size: cover')
  expect(screen.queryByAltText('封面主图')).not.toBeInTheDocument()
